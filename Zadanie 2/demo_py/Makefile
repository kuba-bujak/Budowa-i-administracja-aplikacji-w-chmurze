...
docker_build:
	sudo docker-compose -f docker-compose.yaml --project-name $(PROJECT_NAME) build && \
	sudo docker tag ${PROJECT_NAME}_order-manager wojciech12/$(PROJECT_NAME)
...
start:
	sudo docker-compose -f docker-compose.yaml --project-name $(PROJECT_NAME) up -d

stop:
	sudo docker-compose -f docker-compose.yaml --project-name $(PROJECT_NAME) stop
